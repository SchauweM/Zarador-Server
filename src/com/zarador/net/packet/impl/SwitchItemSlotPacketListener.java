			...
			if (player.isBanking() && player.getInterfaceId() == 5292) {
				...
				if (player.getBankSearchingAttributes().isSearchingBank()) {
					if (player.swapMode()) {
						...
						player.getBankSearchingAttributes().getSearchedBank().swap(fromSlot, toSlot).sortItems();
					}
		...
		if (player.isBanking() && player.getInterfaceId() == 5292) {
			...
			for (int i = 0; i < Bank.BANK_TAB_INTERFACES.length; i++) {
				...
				if (toBankTab) {
					...
					player.getBank(Bank.getTabForItem(player, item.getId())).switchItem(player.getBank(bankTab), item,
							slot, true, false);
