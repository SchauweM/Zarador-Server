	...
	public static boolean handleObelisk(int objectId) {
		...
		if (index >= 0) {
			if (!OBELISK_ACTIVATED[index]) {
				...
				obelisks[1] = new GameObject(14825,
						new Position(OBELISK_COORDS[index][0] + 4, OBELISK_COORDS[index][1]));
				obelisks[2] = new GameObject(14825,
						new Position(OBELISK_COORDS[index][0], OBELISK_COORDS[index][1] + 4));
				obelisks[3] = new GameObject(14825,
						new Position(OBELISK_COORDS[index][0] + 4, OBELISK_COORDS[index][1] + 4));
				...
				for (int i = 0; i < obelisks.length; i++) {
					...
					CustomObjects.globalObjectRespawnTask(obelisks[i],
							new GameObject(OBELISK_IDS[index], new Position(obeliskX, obeliskY)), 4);
				}
