...
public class WarriorsGuild {
	 ...
	 */
	private static final int[][] ARMOR_DATA = { { 1075, 1117, 1155, 4278 }, { 1067, 1115, 1153, 4279 },
			{ 1069, 1119, 1157, 4280 }, { 1077, 1125, 1165, 4281 }, { 1071, 1121, 1159, 4282 },
			{ 1073, 1123, 1161, 4283 }, { 1079, 1127, 1163, 4284 } };
			...
			for (int i = 0; i < ARMOR_DATA.length; i++) {
				for (int f = 0; f < ARMOR_DATA[0].length; f++) {
					if (item.getId() == ARMOR_DATA[i][f]) {
						if (player.getInventory().contains(ARMOR_DATA[i][0])
								&& player.getInventory().contains(ARMOR_DATA[i][1])
								&& player.getInventory().contains(ARMOR_DATA[i][2])) {
	 ...
	 * 
	 * @param player
	 *            The player to handle the drop for
	 * @param npc
	 *            The npc which will drop something
	 */
	public static void handleDrop(Player player, NPC npc) {
		...
		if (npc.getId() >= 4278 && npc.getId() <= 4284) {
			...
			if (armour != null) {
				for (int i : armour)
					GroundItemManager.spawnGroundItem(player, new GroundItem(new Item(i), npc.getPosition().copy(),
							player.getUsername(), false, 80, true, 80));
				player.getMinigameAttributes().getWarriorsGuildAttributes().setSpawnedArmour(false);
				GroundItemManager.spawnGroundItem(player, new GroundItem(new Item(8851, getTokenAmount(npc.getId())),
						npc.getPosition().copy(), player.getUsername(), false, 80, true, 80));
				armour = null;
			}
	 ...
	 * 
	 * @param player
	 *            The player to search items for
	 ...
	 * 
	 * @param npc
	 *            The npc to check how many tokens to receive from
	 ...
	 * 
	 * @param player
	 *            The player to check if cyclop is in combat with
	 */
