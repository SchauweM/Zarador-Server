...
public class FightPit {
		...
		for (Player player : playerMap.keySet()) {
			if (player != null) {
				if (getListCount("PLAYING") == 1 && getState(player) != null && getState(player).equals("PLAYING")) {
					...
					// player.moveTo(new Position(WAITING_ROOM_X,
					// WAITING_ROOM_Y, 0));
					// playerMap.remove(player);
					// playerMap.put(player, WAITING);
	...
	public static void viewOrb(Player player) {
		/*
		 * if(!Locations.inPitsWaitRoom(player) || player.viewingOrb ||
		 * Locations.inPits(player)) return; for(int i = 0; i <
		 * org.Desolace.util.Constants.SIDEBAR_INTERFACES.length; i++)
		 * player.getPacketSender().sendTabInterface(i, -1);
		 * player.getPacketSender().sendTabInterface(4, 3209); player.viewingOrb
		 * = true; player.getMovementQueue().setMovementStatus(MovementStatus.
		 * CANNOT_MOVE);
		 */
	}
	...
	public static void cancelOrbView(Player player) {
		/*
		 * if(!player.viewingOrb) return; player.viewingOrb = false; for(int i =
		 * 0; i < org.Desolace.util.Constants.SIDEBAR_INTERFACES.length; i++)
		 * player.getPacketSender().sendTabInterface(i,
		 * org.Desolace.util.Constants.SIDEBAR_INTERFACES[i]);
		 * player.getPacketSender().sendTabInterface(Constants.PRAYER_TAB,
		 * player.getPrayerBook().getInterfaceId());
		 * player.getPacketSender().sendTabInterface(Constants.MAGIC_TAB,
	...
	public static void viewOrbLocation(Player player, Position pos, int cameraAngle) {
		/*
		 * if(!Locations.inPitsWaitRoom(player) || !player.viewingOrb ||
		 * Locations.inPits(player)) return;
		 */
