...
public class MoneyPouch {
		...
		if (validateAmount(plr, amount)) {
			...
			if (addedMoney > Long.MAX_VALUE) {
				...
				plr.getPacketSender().sendMessage(
						"You've added " + currencyFormatter.format(canStore) + " coins to your money pouch.");
				return true;
			} else {
				...
				plr.getPacketSender().sendString(8135, "" + plr.getMoneyInPouch());
				plr.getPacketSender().sendMessage(
						"You've added " + currencyFormatter.format(amount) + " coins to your money pouch.");
				return true;
			}
		} else {
			plr.getPacketSender().sendMessage(
					"You do not seem to have " + currencyFormatter.format(amount) + " coins in your inventory.");
			return false;
		}
	}
	...
	public static boolean withdrawMoney(Player plr, long amount) {
		...
		if (amount <= 0)
			return false;
		if (plr.getLocation() == Location.WILDERNESS || plr.getCombatBuilder().isAttacking()
				|| plr.getCombatBuilder().isBeingAttacked()) {
		...
		if ((plr.getInventory().getAmount(995) + amount) < Integer.MAX_VALUE) {
			...
			plr.getPacketSender().sendString(8135, "" + plr.getMoneyInPouch());
			plr.getPacketSender()
					.sendMessage("You've withdrawn " + currencyFormatter.format(amount) + " coins from your pouch.");
			if (allowWithdraw)
	 ...
	 * @return false Returns false if the player does not have the coins in
	 *         their inventory
	 */
