...
public class Summoning {
	...
	public void summon(final FamiliarData familiar, boolean renew, boolean login) {
		...
		if (login || player.getSkillManager().getMaxLevel(Skill.SUMMONING) >= familiar.levelRequired) {
			...
			if (!login) {
				if (player.getSkillManager().getCurrentLevel(Skill.SUMMONING) >= familiar.summoningPointsRequired) {
					...
					if (renew && getFamiliar() != null) {
						...
						getFamiliar().setDeathTimer((int) (SummoningData.getFollowerTimer(familiar.npcId)
								* (Skillcape_Data.SUMMONING.isWearingCape(player)
										|| Skillcape_Data.MASTER_SUMMONING.isWearingCape(player) ? 1.15 : 1)));
			...
			int deathTime = login && getFamiliar() != null && getFamiliar().getDeathTimer() > 0
					? getFamiliar().getDeathTimer() : SummoningData.getFollowerTimer(familiar.npcId);
					...
					for (Item t : bob.getValidItems()) {
						...
						Logs.log(player, "drops",
								new String[] {
										...
										"Item: "+t.getDefinition().getName()+" ("+t.getId()+")",
										"Amount: "+t.getAmount()+"",
										"Tracking: "+address+""
								});
					}
	...
	public void clearInterface() {
		...
		player.getPacketSender().sendNpcHeadOnInterface(60, 54021); // 60 =
																	// invisable
																	// head to
																	// remove it
