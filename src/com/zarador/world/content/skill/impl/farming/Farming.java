...
import com.zarador.world.content.skill.impl.farming.seed.TreeSeedType;
				...
				if (farming.patch.getSeedType().getSeedClass() == SeedClass.TREES) {
					if (option == 1) {
						if (!farming.patch.hasAttribute(PatchAttribute.CHECKED_HEALTH)) {
							...
							player.getSkillManager().addSkillExperience(farming.getSkill(), experience);
			...
			if (seedType != null) {	
				...
				TaskManager.submit(new Task(2) {
					...
					public void execute() {
						...
						//player.getFields().setAnimationBusy(false);
						player.getSkillManager().addSkillExperience(Skill.FARMING, finalSeedType.getExperience()[0]);
						stop();
					}
