...
import com.zarador.model.input.impl.EnterAmountOfBarsToSmelt;
...
public class SmithingData {
	...
	public static final int[][] SmeltData = { { 2349, 438, 436, 1, 7 }, // Bronze
																			// bar
			{ 2351, 440, -1, 15, 13 }, // Iron bar
			{ 2355, 442, -1, 20, 14 }, // Silver bar
			{ 2353, 440, 453, 30, 18 }, // Steel bar
			{ 2357, 444, -1, 40, 23 }, // Gold bar
	...
	public static boolean canSmelt(Player player, int barId) {
		...
		if (!hasOres(player, barId)) {
			...
			if (player.getOres()[0] > 0 && player.getOres()[1] > 0 && player.getOres()[1] != 453) {
				requirement = "To make " + anOrA(barId) + " " + new Item(barId).getDefinition().getName()
						+ ", you need some "
						+ new Item(player.getOres()[0]).getDefinition().getName().replace(" ore", "") + " and "
						+ new Item(player.getOres()[1]).getDefinition().getName() + ".";
			} else if (player.getOres()[0] > 0 && player.getOres()[1] == -1) {
				requirement = "To make " + anOrA(barId) + " " + new Item(barId).getDefinition().getName()
						+ ", you need some " + new Item(player.getOres()[0]).getDefinition().getName() + ".";
			} else if (player.getOres()[0] > 0 && player.getOres()[1] == 453) { // The
																				// bar
																				// uses
																				// custom
																				// coal
				...
				requirement = "To make " + anOrA(barId) + " " + new Item(barId).getDefinition().getName()
						+ ", you need some "
						+ new Item(player.getOres()[0]).getDefinition().getName().replace(" ore", "") + " and "
						+ getCoalAmount(barId) + " " + new Item(player.getOres()[1]).getDefinition().getName()
						+ " ores.";
			}
	...
	public static void makeMithInterface(Player player) {
		...
		player.getPacketSender().sendSmithingData(1109, 0, 1121, 1); // chain
																		// body
		...
		player.getPacketSender().sendSmithingData(1159, 1, 1122, 1); // full
																		// helm
		...
		player.getPacketSender().sendSmithingData(1181, 2, 1122, 1); // Sq.
																		// Shield
	...
	public static void makeAddyInterface(Player player) {
		...
		player.getPacketSender().sendSmithingData(1111, 0, 1121, 1); // chain
																		// body
		...
		player.getPacketSender().sendSmithingData(1161, 1, 1122, 1); // full
																		// helm
		...
		player.getPacketSender().sendSmithingData(1183, 2, 1122, 1); // Sq.
																		// Shield
	...
	public static void makeRuneInterface(Player player) {
		...
		player.getPacketSender().sendSmithingData(1113, 0, 1121, 1); // chain
																		// body
		...
		player.getPacketSender().sendSmithingData(1163, 1, 1122, 1); // full
																		// helm
		...
		player.getPacketSender().sendSmithingData(1185, 2, 1122, 1); // Sq.
																		// Shield
	...
	public static int getBarAmount(Item item) {
		String name = item.getDefinition().getName().toLowerCase();
		if (name.contains("scimitar") || name.contains("claws") || name.contains("longsword")
				|| name.contains("sq shield") || name.contains("full helm")) {
			return 2;
