package com.zarador.world.content.skill.impl.fletching;
...
import com.zarador.model.input.impl.EnterAmountOfBowsToString;
import com.zarador.model.input.impl.EnterAmountToFletch;
...
public class Fletching {
	...
	public static int getBoltPrimary(int item1, int item2) {
		return item1 == 314 ? item2 : item1;
	}

	public static void makeBolts(final Player player, int item1, int item2) {
		player.getSkillManager().stopSkilling();
		BoltData bolt = BoltData.forBolt(getBoltPrimary(item1, item2));
		if (bolt != null) {
			if (player.getSkillManager().getCurrentLevel(Skill.FLETCHING) >= bolt.getLevelReq()) {
				if (player.getInventory().getAmount(bolt.getItem1()) >= 15
						&& player.getInventory().getAmount(bolt.getItem2()) >= 15) {
					player.getInventory().delete(new Item(bolt.getItem1()).setAmount(15),
							player.getInventory().getSlot(bolt.getItem1()), true);
				...
				player.getPacketSender().sendMessage(
						"You need a Fletching level of at least " + bolt.getLevelReq() + " to fletch this.");
			}
		}
	}
