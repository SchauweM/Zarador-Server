...
public abstract class AbstractSkill {
	...
	public boolean success(int requirement) {
        int factor = MathUtil.random(getPlayer().getSkillManager().getMaxLevel(getSkill()));
        int fluke = MathUtil.random(requirement == 1 ? 3 : requirement);
        return factor > fluke;
	}
	...
	public boolean success(int requirement, int toolRequirement) {
		...
		int successChance = MathUtil.random((int) (Math.ceil((level - requirement))));
		int roll = MathUtil.random(requirement);
