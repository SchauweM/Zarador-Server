...
import com.zarador.engine.task.impl.PoisonImmunityTask;
...
public class Consumables {
	...
	public static void doTheBrew(Player player) {
		...
		if (player.getSkillManager()
				.getCurrentLevel(Skill.forId(1)) > (player.getSkillManager().getMaxLevel(Skill.forId(1)) * 1.2 + 1)) {
			player.getSkillManager().setCurrentLevel(Skill.forId(1),
	...
	public static void handleSpecRestore(Player player) {
	...
	public static void handleNormalRestore(Player player) {
		player.performAnimation(new Animation(829));
		player.getInventory().refreshItems();
		for (int i = 0; i <= 24; i++) {
			if (i == 3 || i == 5)
	...
	public static void handleSuperEnergyRestore(Player player) {
		player.getInventory().refreshItems();
	...
	public static void handleEnergyRestore(Player player) {
		player.getInventory().refreshItems();
		player.setRunEnergy(player.getRunEnergy() + 15);
		if (player.getRunEnergy() > 100)
			player.setRunEnergy(100);
	}
	...
	private enum FoodType {
				...
				40), THIRD_CAKE_SLICE(new Item(1895), 40), BANDAGES(new Item(14640), 120), JANGERBERRIES(new Item(247),
						20), WORM_CRUNCHIES(new Item(2205), 70), EDIBLE_SEAWEED(new Item(403), 40), ANCHOVIES(
								...
								10), SHRIMPS(new Item(315), 30), SARDINE(new Item(325), 40), COD(new Item(339),
										70), TROUT(new Item(333), 70), PIKE(new Item(351), 80), SALMON(new Item(329),
												90), TUNA(new Item(361), 100), LOBSTER(new Item(379), 120), BASS(
														new Item(365),
														130), SWORDFISH(new Item(373), 140), MEAT_PIZZA(new Item(2293),
																145), MONKFISH(new Item(7946), 160), SHARK(
																		new Item(385),
																		200), SEA_TURTLE(new Item(397), 210), MANTA_RAY(
		 ...
		 */
		POTATO(new Item(1942), 10), BAKED_POTATO(new Item(6701), 40), POTATO_WITH_BUTTER(new Item(6703),
				140), CHILLI_POTATO(new Item(7054), 140), EGG_POTATO(new Item(7056), 160), POTATO_WITH_CHEESE(
						new Item(6705), 160), MUSHROOM_POTATO(new Item(7058), 200), TUNA_POTATO(new Item(7060), 220),

		/*
		 * Fruit food types which a player can get by picking from certain trees
		 * or hand-making them (such as pineapple chunks/rings).
		 */
		SPINACH_ROLL(new Item(1969), 20), BANANA(new Item(1963), 20), BANANA_(new Item(18199), 20), CABBAGE(
				new Item(1965), 20), ORANGE(new Item(2108), 20), PINEAPPLE_CHUNKS(new Item(2116),
						20), PINEAPPLE_RINGS(new Item(2118), 20), PEACH(new Item(6883), 80),

		/*
		 * Dungeoneering food types, which you can get in the Dungeoneering
		 * skill dungeons.
		 */
		HEIM_CRAB(new Item(18159), 20), RED_EYE(new Item(18161), 50), DUSK_EEL(new Item(18163), 70), GIANT_FLATFISH(
				new Item(18165),
				100), SHORT_FINNED_EEL(new Item(18167), 120), WEB_SNIPPER(new Item(18169), 150), BOULDABASS(
						new Item(18171), 170), SALVE_EEL(new Item(18173), 200), BLUE_CRAB(new Item(18175), 220),
	...
	public static boolean isPotion(int itemId) {
		...
		return pot.contains("(6)") || pot.contains("(5)") || pot.contains("(4)") || pot.contains("(3)")
	...
	public static void handlePotion(final Player player, final int itemId, final int slot) {
		...
		// player.getPacketSender().sendMessage("Potions have been disabled in
		// this clan battle.");
		// return;
		// }
		// if(ClanWars.Rules.COMBAT_FOOD.getToggle() == false &&
		// healingPotion(itemId)) {
		// player.getPacketSender().sendMessage("Since food has been disabled in
		// this clan battle,
		...
		if (player.getPotionTimer().elapsed(1800)) {
			 ...
			 */
			case 2428:
				drinkStatPotion(player, itemId, 121, slot, 0, false); // attack
																		// pot 4
				break;
			case 121:
				drinkStatPotion(player, itemId, 123, slot, 0, false); // attack
																		// pot 3
				break;
			case 123:
				drinkStatPotion(player, itemId, 125, slot, 0, false); // attack
																		// pot2
				break;
			case 125:
				drinkStatPotion(player, itemId, 229, slot, 0, false); // attack
																		// pot 1
				break;
			/*
			 * Defence potions
			 */
			case 2432:
				drinkStatPotion(player, itemId, 133, slot, 1, false); // Defence
																		// pot 4
				break;
			case 133:
				drinkStatPotion(player, itemId, 135, slot, 1, false); // Defence
																		// pot 3
				break;
			case 135:
				drinkStatPotion(player, itemId, 137, slot, 1, false); // Defence
																		// pot 2
				break;
			case 137:
				drinkStatPotion(player, itemId, 229, slot, 1, false); // Defence
																		// pot 1
				break;
			/*
			 * Strength potions
			 */
			case 113:
				drinkStatPotion(player, itemId, 115, slot, 2, false); // Strength
																		// pot 4
				break;
			case 115:
				drinkStatPotion(player, itemId, 117, slot, 2, false); // Strength
																		// pot 3
				break;
			case 117:
				drinkStatPotion(player, itemId, 119, slot, 2, false); // Strength
																		// pot 2
				break;
			case 119:
				drinkStatPotion(player, itemId, 229, slot, 2, false); // Strength
																		// pot 1
				break;
			/*
			 * Antipoison
			...
			case 3016: // Super energy 4
				...
				handleSuperEnergyRestore(player);
				break;
			case 3008: // Energy potion 4
				player.getInventory().getItems()[slot] = new Item(itemId + 2, 1);
				handleEnergyRestore(player);
				break;
			 ...
			 */
			case 3032:
				drinkStatPotion(player, itemId, 3034, slot, 16, false); // Agility
																		// pot 4
				break;
			case 3034:
				drinkStatPotion(player, itemId, 3036, slot, 16, false); // Agility
																		// pot 3
				break;
			case 3036:
				drinkStatPotion(player, itemId, 3038, slot, 16, false); // Agility
																		// pot 2
				break;
			case 3038:
				drinkStatPotion(player, itemId, 229, slot, 16, false); // Agility
																		// pot 1
				break;
			/*
			 * Prayer potions
			 ...
			 */
			case 14838:
				drinkStatPotion(player, itemId, 14840, slot, 12, false); // Crafting
																			// pot
																			// 4
				break;
			case 14840:
				drinkStatPotion(player, itemId, 14842, slot, 12, false); // Crafting
																			// pot
																			// 3
				break;
			case 14842:
				drinkStatPotion(player, itemId, 14844, slot, 12, false); // Crafting
																			// pot
																			// 2
				break;
			case 14844:
				drinkStatPotion(player, itemId, 229, slot, 12, false); // Crafting
																		// pot 1
				break;
			/*
			...
			case 2436:
				...
				break;
			case 145:
				drinkStatPotion(player, itemId, 147, slot, 0, true); // Super
																		// Attack
																		// pot 3
				break;
			case 147:
				drinkStatPotion(player, itemId, 149, slot, 0, true); // Super
																		// Attack
																		// pot 2
				break;
			case 149:
				drinkStatPotion(player, itemId, 229, slot, 0, true); // Super
																		// Attack
																		// pot 1
				break;
			 ...
			 */
			case 2438:
				drinkStatPotion(player, itemId, 151, slot, 10, false); // Fishing
																		// pot 4
				break;
			case 151:
				drinkStatPotion(player, itemId, 153, slot, 10, false); // Fishing
																		// pot 3
				break;
			case 153:
				drinkStatPotion(player, itemId, 155, slot, 10, false); // Fishing
																		// pot 2
				break;
			case 155:
				drinkStatPotion(player, itemId, 229, slot, 10, false); // Fishing
																		// pot 1
				break;
			/*
			 * Hunter potions
			 */
			case 9998:
				drinkStatPotion(player, itemId, 10000, slot, 22, false); // Hunter
																			// pot
																			// 4
				break;
			case 10000:
				drinkStatPotion(player, itemId, 10002, slot, 22, false); // Hunter
																			// pot
																			// 3
				break;
			case 10002:
				drinkStatPotion(player, itemId, 10004, slot, 22, false); // Hunter
																			// pot
																			// 2
				break;
			case 10004:
				drinkStatPotion(player, itemId, 229, slot, 22, false); // Hunter
																		// pot 1
				break;
			/*
			 ...
			 */
			case 2440:
				drinkStatPotion(player, itemId, 157, slot, 2, true); // Super
																		// Strength
																		// pot 4
				break;
			case 157:
				drinkStatPotion(player, itemId, 159, slot, 2, true); // Super
																		// Strength
																		// pot 3
				break;
			case 159:
				drinkStatPotion(player, itemId, 161, slot, 2, true); // Super
																		// Strength
																		// pot 2
				break;
			case 161:
				drinkStatPotion(player, itemId, 229, slot, 2, true); // Super
																		// Strength
																		// pot 1
				break;
			/*
			 ...
			 */
			case 14846:
				drinkStatPotion(player, itemId, 14848, slot, 9, false); // Fletching
																		// pot 4
				break;
			case 14848:
				drinkStatPotion(player, itemId, 14850, slot, 9, false); // Fletching
																		// pot 3
				break;
			case 14850:
				drinkStatPotion(player, itemId, 14852, slot, 9, false); // Fletching
																		// pot 2
				break;
			case 14852:
				drinkStatPotion(player, itemId, 229, slot, 9, false); // Fletching
																		// pot 1
				break;
			/*
			...
			case 5943: // Antipoison+ pot 4
				...
				player.getPacketSender().sendMessage("You're now immune to poison for another 518 seconds.");
				break;
			...
			case 11525: // Anti-venom pot 4
				if (player.getVenomDamage() == 0 && player.getPoisonDamage() == 0) {
					player.getPacketSender()
							.sendMessage("<col=ff0000>You currently do not have any venom/poison effect.");
					return;
				}
				player.performAnimation(new Animation(829));
			 ...
			 */
			case 2444:
				drinkStatPotion(player, itemId, 169, slot, 4, false); // Ranging
																		// pot 4
				break;
			case 169:
				drinkStatPotion(player, itemId, 171, slot, 4, false); // Ranging
																		// pot 3
				break;
			case 171:
				drinkStatPotion(player, itemId, 173, slot, 4, false); // Ranging
																		// pot 2
				break;
			case 173:
				drinkStatPotion(player, itemId, 229, slot, 4, false); // Ranging
																		// pot 1
				break;
			/*
			 * Magic potions
			 */
			case 3040:
				drinkStatPotion(player, itemId, 3042, slot, 6, false); // Magic
																		// pot 4
				break;
			case 3042:
				drinkStatPotion(player, itemId, 3044, slot, 6, false); // Magic
																		// pot 3
				break;
			case 3044:
				drinkStatPotion(player, itemId, 3046, slot, 6, false); // Magic
																		// pot 2
				break;
			case 3046:
				drinkStatPotion(player, itemId, 229, slot, 6, false); // Magic
																		// pot 1
				break;
			/*
			 * Zamorak brews
			 ...
			 */
			case 15308:
				drinkExtremePotion(player, 15308, 15309, slot, 0);
				break;
			case 15309:
				drinkExtremePotion(player, 15309, 15310, slot, 0);
				break;
			case 15310:
				drinkExtremePotion(player, 15310, 15311, slot, 0);
				break;
			case 15311:
				drinkExtremePotion(player, 15311, 229, slot, 0);
				break;
			/*
			 * Extreme Strength potion
			 */
			case 15312:
				drinkExtremePotion(player, 15312, 15313, slot, 2);
				break;
			case 15313:
				drinkExtremePotion(player, 15313, 15314, slot, 2);
				break;
			case 15314:
				drinkExtremePotion(player, 15314, 15315, slot, 2);
				break;
			case 15315:
				drinkExtremePotion(player, 15315, 229, slot, 2);
				break;
			/*
			 * Extreme Defence potions
			 */
			case 15316:
				drinkExtremePotion(player, 15316, 15317, slot, 1);
				break;
			case 15317:
				drinkExtremePotion(player, 15317, 15318, slot, 1);
				break;
			case 15318:
				drinkExtremePotion(player, 15318, 15319, slot, 1);
				break;
			case 15319:
				drinkExtremePotion(player, 15319, 229, slot, 1);
				break;
			/*
			 * Extreme Magic potions
			 */
			case 15320:
				drinkExtremePotion(player, 15320, 15321, slot, 6);
				break;
			case 15321:
				drinkExtremePotion(player, 15321, 15322, slot, 6);
				break;
			case 15322:
				drinkExtremePotion(player, 15322, 15323, slot, 6);
				break;
			case 15323:
				drinkExtremePotion(player, 15323, 229, slot, 6);
				break;
			/*
			 * Extreme Ranging potions
			 */
			case 15324:
				drinkExtremePotion(player, 15234, 15325, slot, 4);
				break;
			case 15325:
				drinkExtremePotion(player, 15325, 15326, slot, 4);
				break;
			case 15326:
				drinkExtremePotion(player, 15326, 15327, slot, 4);
				break;
			case 15327:
				drinkExtremePotion(player, 15327, 229, slot, 4);
				break;
			/*
			...
			if (player.getOverloadPotionTimer() > 0) { // Prevents decreasing
														// stats
	...
	public static void drinkStatPotion(final Player player, int potion, int replacePotion, int slot, int skill,
		...
		if (skill == 100) {
			player.getSkillManager().setCurrentLevel(Skill.forId(0),
					player.getSkillManager().getCurrentLevel(Skill.forId(0))
							+ getBoostedStat(player, 0, super_pot, cbPot));
			player.getSkillManager().setCurrentLevel(Skill.forId(1),
					player.getSkillManager().getCurrentLevel(Skill.forId(1))
							+ getBoostedStat(player, 1, super_pot, cbPot));
			player.getSkillManager().setCurrentLevel(Skill.forId(2),
					player.getSkillManager().getCurrentLevel(Skill.forId(2))
							+ getBoostedStat(player, 2, super_pot, cbPot));
		} else {
			Skill sk = Skill.forId(skill);
			player.getSkillManager().setCurrentLevel(sk,
					player.getSkillManager().getCurrentLevel(sk) + getBoostedStat(player, skill, super_pot, cbPot),
					true);
		}
	}

	public static void drinkExtremePotion(final Player player, int potion, int replacePotion, int slot, int skill) {
		if (player.getLocation() != null && player.getLocation() == Location.WILDERNESS) {
			player.getPacketSender().sendMessage("You cannot use this potion in the Wilderness.");
			return;
		}
		...
		player.getInventory().refreshItems();
		player.performAnimation(new Animation(829));
		player.getSkillManager().setCurrentLevel(Skill.forId(skill),
				player.getSkillManager().getCurrentLevel(Skill.forId(skill)) + getExtremePotionBoost(player, skill));
	}
	...
	public static int getBoostedStat(Player player, int skillId, boolean sup, boolean combatPotion) {
		...
		int increaseBy = 0;
		if (sup)
			increaseBy = (int) (player.getSkillManager().getMaxLevel(skill) * 0.2D);
		else
			increaseBy = (int) (player.getSkillManager().getMaxLevel(skill) * (combatPotion ? 0.10D : 0.13D))
					+ 1;
		if (skillId == 6) {
			increaseBy = (int) (player.getSkillManager().getMaxLevel(skill) * (combatPotion ? 0.05D : 0.05D))
					+ 1;
		} else if (skillId == 500) {
			increaseBy = (int) (player.getSkillManager().getMaxLevel(Skill.forId(6))
