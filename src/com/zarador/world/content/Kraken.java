...
public class Kraken {
		...
		player.getKraken().setKrakenStage(KrakenStage.TELEPORTED_IN);
			...
			public void execute() {
				...
				if(tick == spawnTick) {
					...
					whirlpool[2] = new NPC(493, new Position(3700, 5814, player.getPosition().getZ())).setSpawnedFor(player);
					World.register(whirlpool[2]);
					whirlpool[3] = new NPC(493, new Position(3691, 5809, player.getPosition().getZ())).setSpawnedFor(player);
					World.register(whirlpool[3]);
					whirlpool[4] = new NPC(493, new Position(3691, 5814, player.getPosition().getZ())).setSpawnedFor(player);
	...
	public void spawnKraken(Player player, NPC npc) {
		kraken = new NPC(502, npc.getPosition()).setSpawnedFor(player);
	...
	public Kraken() {
		...
		this.kraken = null;
		this.krakenStage = KrakenStage.TELEPORTED_IN;
	}
}