package com.zarador.world.content.combat.prayer;
...
import com.zarador.world.content.ItemsKeptOnDeath;
...
public class CurseHandler {
	...
	public static void activateCurse(Player player, CurseData curse) {
		...
		if (!player.getDragonScimitarTimer().elapsed(5000)) {
			player.getPacketSender()
					.sendMessage("You have been damaged badly with a Dragon Scimitar! Try again in 5 seconds.");
			CurseHandler.deactivateAll(player);
			PrayerHandler.deactivateAll(player);
			return;
		}
		...
		if (player.getSkillManager().getMaxLevel(Skill.PRAYER) < (curse.requirement * 10)) {
			player.getPacketSender().sendConfig(curse.configId, 0);
			// DialogueManager.sendOneStringStatement(p, " " + "You need a
			// @blu@Prayer <col=0>level of " +
			// pd.levelRequirement +" to use @blu@" + pd.getPrayerName());
		...
		/*
		 * if (player.isPrayerInjured()) { if (curse == CurseData.DEFLECT_MAGIC
		 * || curse == CurseData.DEFLECT_MELEE || curse ==
		 * CurseData.DEFLECT_MISSILES || curse == CurseData.DEFLECT_SUMMONING) {
		 * player.getPacketSender().sendMessage(
		 * "You have been injured and cannot use this prayer!");
		...
		PROTECT_ITEM(50, .1, 32503, 610, new PrayerAnimation(new Animation(12567), new Graphic(2213))), SAP_WARRIOR(50,
				.3, 32505, 611), SAP_RANGER(52, .3, 32507, 612), SAP_MAGE(54, .3, 32509, 613), SAP_SPIRIT(56, .3, 32511,
						614), BERSERKER(59, .4, 32513, 615, new PrayerAnimation(new Animation(12589),
								new Graphic(2266))), DEFLECT_SUMMONING(62, .5, 32515, 616), DEFLECT_MAGIC(65, .5, 32517,
										617), DEFLECT_MISSILES(68, .5, 32519, 618), DEFLECT_MELEE(71, .5, 32521,
												619), LEECH_ATTACK(74, 1, 32523, 620), LEECH_RANGED(76, 1, 32525,
														621), LEECH_MAGIC(78, 1, 32527, 622), LEECH_DEFENCE(80, 1,
																32529,
																623), LEECH_STRENGTH(82, 1, 32531, 624), LEECH_ENERGY(
																		84, 1, 32533, 625), LEECH_SPECIAL_ATTACK(86, 1,
																				32535, 626), WRATH(89, 1.2, 32537,
																						627), SOUL_SPLIT(92, 1.5, 32539,
																								628), TURMOIL(95, 3,
																										32541, 629,
																										new PrayerAnimation(
																												new Animation(
	...
	public static final CurseData[] OVERHEAD_CURSES = { CurseData.DEFLECT_MAGIC, CurseData.DEFLECT_MISSILES,
			CurseData.DEFLECT_MELEE, CurseData.DEFLECT_SUMMONING, CurseData.WRATH, CurseData.SOUL_SPLIT };
