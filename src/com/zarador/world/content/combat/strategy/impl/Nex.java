...
public class Nex implements CombatStrategy {
					...
					TaskManager.submit(new Task(1, NEX, false) {
						...
						public void execute() {
							if (ticks == 10)
								...
								double dist = p_.getPosition().distanceToPoint(NEX.getPosition().getX(),
	...
	public static boolean checkAttack(Player p, int npc) {
		if (npc == 13447) {
			for (int i = 0; i < magesAttackable.length; i++) {
				if (magesAttackable[i] && !magesKilled[i]) {
					int index = 13451 + i;
					p.getPacketSender().sendMessage("You need to kill " + NpcDefinition.forId(index).getName()
		...
		if (!magesAttackable[index] && !magesKilled[index]) {
			p.getPacketSender()
					.sendMessage("" + NpcDefinition.forId(npc).getName() + " is currently being protected by Nex!");
			return false;
		}
		return true;
	}
			...
			public void execute() {
				...
				for (Player p2 : Misc.getCombinedPlayerList(p)) {
					...
					if (p2.getPosition().distanceToPoint(p.getPosition().getX(), p.getPosition().getY()) == 1
							&& p2.getConstitution() > 0 && p2.getLocation() == Location.GODWARS_DUNGEON) {
						cough(p2);
					}
				}
				ticks++;
