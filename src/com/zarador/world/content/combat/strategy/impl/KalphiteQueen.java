				...
				protected void execute() {
					if (tick == 1) {
						for (Player toAttack : list) {
							if (toAttack != null
									&& Locations.goodDistance(KALPHITE_QUEEN.getPosition(), toAttack.getPosition(), 7)
									&& toAttack.getConstitution() > 0) {
								new Projectile(KALPHITE_QUEEN, toAttack, secondForm() ? 279 : 280, 44, 3, 43, 43, 0)
										.sendProjectile();
							}
						}
					} else if (tick == 3) {
						...
						for (Player toAttack : list) {
							if (toAttack != null
								...
								CombatType cbType = (secondForm() && Misc.getRandom(5) <= 3 ? CombatType.RANGED
										: CombatType.MAGIC);
								KALPHITE_QUEEN.getCombatBuilder().setVictim(toAttack);
								new CombatHit(KALPHITE_QUEEN.getCombatBuilder(),
										new CombatContainer(KALPHITE_QUEEN, toAttack, 1, cbType, true)).handleAttack();
							}
						}
