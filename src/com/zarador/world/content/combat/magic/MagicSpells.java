		...
		case BONES_TO_BANANAS:
			...
			if (!p.getInventory().contains(item.getId())) {
				p.getPacketSender().sendMessage(
						"You do not have any " + item.getDefinition().getName() + "" + sa + " in your inventory.");
				return true;
			}
			p.getInventory().delete(557, spell == BONES_TO_PEACHES ? 4 : 2)
					.delete(555, spell == BONES_TO_PEACHES ? 4 : 2).delete(561, spell == BONES_TO_PEACHES ? 2 : 1);
			int i = 0;
			...
			if (!p.getLastVengeance().elapsed(30000)) {
				p.getPacketSender()
						.sendMessage("You must wait another "
								+ Misc.getTimeLeft(p.getLastVengeance().getTime(), 30, TimeUnit.SECONDS)
								+ " seconds before you can cast vengeance again.");
				return true;
			}
