...
public final class CharacterList<E extends Character> implements Iterable<E> {
	...
	/**
	 * The backing array of {@link CharacterList}s within this collection.
	 */
	private E[] characters;

	/**
	 * The queue containing all of the slots that {@link CharacterList}s were
		...
		if (e.isPlayer()) {
			...
			if (player.getSession().getState() != SessionState.LOGGING_OUT) {
				...
				if (!player.forceOffline) {
					...
					System.err.println("Player state not set to logging out, attempting to force logout player: "
							+ player.getUsername());
