...
public class PlayerSkillsTask extends Task {
	 ...
	 * 
	 * @param player
	 *            The player to normalize skills for.
	 */
	public PlayerSkillsTask(Player player) {
		super(player.getCurseActive()[CurseHandler.BERSERKER] ? TIME_WITH_BERSERKER_CURSE
				: TIME_WITHOUT_BERSERKER_CURSE, player, false);
		this.player = player;
	}
	...
	public void execute() {
		...
		if (getDelay() == TIME_WITHOUT_BERSERKER_CURSE && player.getCurseActive()[CurseHandler.BERSERKER]
				|| !player.getCurseActive()[CurseHandler.BERSERKER] && getDelay() == TIME_WITH_BERSERKER_CURSE)
		...
		if (!player.isDying() && player.getConstitution() > 0 && player.getOverloadPotionTimer() == 0) {
			for (Skill skill : Skill.values()) {
				if (player.getSkillManager().getCurrentLevel(skill) != player.getSkillManager().getMaxLevel(skill)
						&& skill != Skill.PRAYER && skill != Skill.SUMMONING) {
