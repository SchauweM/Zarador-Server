...
import com.zarador.model.input.impl.ItemSearch;
...
public class Bank extends ItemContainer {
	...
	public Bank open() {
		...
		getPlayer().getPacketSender().sendConfig(115, getPlayer().withdrawAsNote() ? 1 : 0)
				...
				.sendConfig(117,
						(getPlayer().getBankSearchingAttributes().isSearchingBank()
								&& getPlayer().getBankSearchingAttributes().getSearchedBank() != null) ? 1 : 0)
	...
	public Bank switchItem(ItemContainer to, Item item, int slot, boolean sort, boolean refresh) {
		if (!getPlayer().isBanking() || getPlayer().getInterfaceId() != 5292
				|| to instanceof Inventory && !(getPlayer().getBank(getPlayer().getCurrentBankTab())
						.contains(item.getId()) || getPlayer().getBankSearchingAttributes().getSearchedBank() != null
								&& getPlayer().getBankSearchingAttributes().getSearchedBank().contains(item.getId()))) {
			getPlayer().getPacketSender().sendClientRightClickRemoval();
			return this;
		}
		...
		if (getPlayer().getBankSearchingAttributes().isSearchingBank()
			...
			getPlayer().getBankSearchingAttributes().getSearchedBank().delete(item);
			getPlayer().getBankSearchingAttributes().getSearchedBank().open();
		} else {
	...
	public Bank refreshItems() {
		Bank bank = getPlayer().getBankSearchingAttributes().isSearchingBank()
				&& getPlayer().getBankSearchingAttributes().getSearchedBank() != null
						? getPlayer().getBankSearchingAttributes().getSearchedBank() : this;
	...
	public static void sendTabs(Player player) {
		 ...
		 * if(isEmpty(player.getBank(i)) || moveRest) { int j = i+2 > 8 ? 8 :
		 * i+2; player.setBank(i, player.getBank(j)); player.setBank(j, new
		 * Bank(player)); moveRest = true; } }
		 */
	...
	public static void depositItems(Player p, ItemContainer from, boolean ignoreReqs) {
		...
		for (Item it : from.getValidItems()) {
			...
			if (p.getBankSearchingAttributes().isSearchingBank()
					&& p.getBankSearchingAttributes().getSearchedBank() != null) {
	...
	public static class BankSearchAttributes {
		...
		public static void beginSearch(Player player, String searchSyntax) {
			...
			player.getBankSearchingAttributes().setSearchingBank(true).setSearchSyntax(searchSyntax);
			...
			player.getPacketSender().sendString(27002, Integer.toString(player.getCurrentBankTab())).sendString(27000,
					"1");
			player.getBankSearchingAttributes().setSearchedBank(new Bank(player));
		...
		public static void addItemToBankSearch(Player player, Item item) {
			if (player.getBankSearchingAttributes().getSearchSyntax() != null) {
				if (item.getDefinition().getName().toLowerCase()
						.contains(player.getBankSearchingAttributes().getSearchSyntax())) {
					if (player.getBankSearchingAttributes().getSearchedBank().getFreeSlots() == 0)
						return;
					player.getBankSearchingAttributes().getSearchedBank().add(item, true);
				}
			}
		}
		...
		public static void stopSearch(Player player, boolean openBank) {
			player.getPacketSender().sendClientRightClickRemoval();
			player.getBankSearchingAttributes().setSearchedBank(null).setSearchingBank(false).setSearchSyntax(null);
			player.setCurrentBankTab(0).setNoteWithdrawal(false);
			player.getPacketSender().sendString(27002, Integer.toString(0)).sendString(27000, "1").sendConfig(117, 0)
					.sendString(5383, "          The Bank of Argos");
			if (openBank)
		...
		public static void withdrawFromSearch(Player player, Item item) {
			if (player.isBanking() && player.getBankSearchingAttributes().isSearchingBank()) {
