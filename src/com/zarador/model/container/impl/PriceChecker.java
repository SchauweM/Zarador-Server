...
public class PriceChecker extends ItemContainer {
		...
		for (int i = 0; i < frames.length; i++) {
			...
			if (slotOccupied) {
				...
				if (it.getDefinition().isStackable()) {
					...
					getPlayer().getPacketSender().sendString(frame,
							"" + it.getDefinition().getValue() + " x" + itemAmount);
					...
					getPlayer().getPacketSender().sendString(getFrame(i, 1),
							"" + Misc.insertCommasToNumber(Integer.toString(it.getDefinition().getValue())) + "");
			...
			getPlayer().getPacketSender().sendString(18351,
					"" + Misc.insertCommasToNumber(Integer.toString(calculateTotalWealth())));
		}
		if (itemFound) {
	...
	private static final int frames[][] = { { 0, 18353, 18246 }, { 1, 18356, 18500 }, { 2, 18359, 18501 },
			{ 3, 18362, 18502 }, { 4, 18365, 18503 }, { 5, 18368, 18504 }, { 6, 18371, 18505 }, { 7, 18374, 18506 },
			{ 8, 18377, 18507 }, { 9, 18380, 18508 }, { 10, 18383, 18509 }, { 11, 18386, 18510 }, { 12, 18389, 18511 },
			{ 13, 18392, 18512 }, { 14, 18395, 18513 }, { 15, 18398, 18514 }, { 16, 18401, 18515 },
			{ 17, 18404, 18516 }, { 18, 18407, 18517 }, { 19, 18410, 18518 } };
