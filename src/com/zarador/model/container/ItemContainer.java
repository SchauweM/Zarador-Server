package com.zarador.model.container;
...
import com.zarador.model.container.impl.Inventory;
...
public abstract class ItemContainer {
	...
	public ItemContainer add(Item item, boolean refresh) {
		...
		if (ItemDefinition.forId(item.getId()).isStackable() || stackType() == StackType.STACKS) {
				...
				if (getPlayer().getStaffRights() != StaffRights.OWNER && getPlayer().getStaffRights() != StaffRights.MANAGER && getPlayer().getStaffRights() != StaffRights.ADMINISTRATOR) {
					...
					GroundItemManager.spawnGroundItem(player,
							new GroundItem(item, player.getPosition().copy(), player.getUsername(),
									player.getHostAddress(), false, 120,
			...
			int existingAmount = items[slot].getId() == item.getId() ? items[slot].getAmount() : 0;
			long totalAmount = existingAmount + item.getAmount();
			if (totalAmount > Integer.MAX_VALUE) {
				...
				items[slot].setId(item.getId());
				items[slot].setAmount(existingAmount + item.getAmount());
			}
		} else {
